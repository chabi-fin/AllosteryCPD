...
do
        ...
        if [ -f closed_holo.cpt ]; then

                gmx_mpi mdrun -cpi closed_holo -deffnm closed_holo -nb gpu -update gpu -pme gpu -pin off -ntomp $SLURM_CPUS_PER_TASK -nobackup
		...
		cp -r ${home}/closed_holo.pdb ${home}/amber14sb_ip6.ff ${current}/setup
                cd ${current}/setup

                ### Set up initial system by generating box, solvating etc.
                # Generate topology using .pdb of capped residue
                echo "1 1 1" | gmx pdb2gmx -ff amber14sb_ip6 -f closed_holo.pdb -o complex_only.gro -water tip3p -nobackup -ignh -his
		...
		cd ${current}/equilibrate
                ...
                cp ${prev}/md_run/closed_holo.gro ${current}/equilibrate/complex_initial.gro
	...
	# Production run   
	gmx grompp -f ${MDP}/Production.mdp -c npt.gro -r npt.gro -t npt.cpt -p topol.top -o closed_holo.tpr -n index.ndx -nobackup
        gmx_mpi mdrun -deffnm closed_holo -nb gpu -update gpu -pme gpu -pin off --ntomp $SLURM_CPUS_PER_TASK -nobackup

        if [ -f closed_holo.gro ]; then

	        ### Post processing
        	# Centering and fitting of trajectory
	        echo "24 24 24" | gmx trjconv -f closed_holo.xtc -s closed_holo.tpr -center yes -pbc cluster -o centered_traj.xtc -n index.ndx -nobackup

	        echo "4 24" | gmx trjconv -f centered_traj.xtc -s closed_holo.tpr -fit rot+trans -o fitted_traj.xtc -n index.ndx -nobackup
