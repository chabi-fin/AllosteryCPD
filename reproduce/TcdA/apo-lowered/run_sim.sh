...
do
        ...
        if [ -f closed_apo.cpt ]; then

                gmx_mpi mdrun -cpi closed_apo -deffnm open_apo -nb gpu -update gpu -pme gpu -pin off -ntomp $SLURM_CPUS_PER_TASK -nobackup
		...
		cp -r ${home}/closed_apo.pdb ${home}/amber14sb.ff ${current}/setup
                cd ${current}/setup

                ### Set up initial system by generating box, solvating etc.
                # Generate topology using .pdb of capped residue
                echo "1 1 1" | gmx pdb2gmx -ff amber14sb -f closed_apo.pdb -o cpd_only.gro -water tip3p -nobackup -ignh -his
		...
		cd ${current}/equilibrate
                ...
                cp ${prev}/md_run/closed_apo.gro ${current}/equilibrate/cpd_initial.gro
	...
	gmx grompp -f ${MDP}/Production.mdp -c npt.gro -r npt.gro -t npt.cpt -p topol.top -o closed_apo.tpr -nobackup
        gmx_mpi mdrun -deffnm closed_apo -nb gpu -update gpu -pme gpu -pin off --ntomp $SLURM_CPUS_PER_TASK -nobackup

        if [ -f closed_apo.gro ]; then

                ### Post processing
                # Centering and fitting of trajectory
                echo "1 1" | gmx trjconv -f closed_apo.xtc -s closed_apo.tpr -pbc mol -center yes -o centered_traj.xtc -nobackup
                echo "4 1" | gmx trjconv -f centered_traj.xtc -s closed_apo.tpr -fit rot+trans -o fitted_traj.xtc -nobackup
