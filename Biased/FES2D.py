...
def plot_2dfes(fes, vec_open, vec_closed, fig_path, fsum=None, 
    ...
    # Get Free energy values of the reference states
    o_ref_x, o_ref_y = (np.dot(vec_open, vec_open), np.dot(vec_open, vec_closed))
    distances_open = np.sqrt((x - o_ref_x)**2 + (y - o_ref_y)**2)
    nearest_open_index = np.argmin(distances_open)
    print(f"\nF(open_ref) = {z[nearest_open_index]}")

    c_ref_x, c_ref_y = (np.dot(vec_open, vec_closed), np.dot(vec_closed, vec_closed))
    distances_closed = np.sqrt((x - c_ref_x)**2 + (y - c_ref_y)**2)
    nearest_closed_index = np.argmin(distances_closed)
    print(f"\nF(closed_ref) = {z[nearest_closed_index]}")
    ...
    if angle_coord:
        ax.scatter(calc_theta(vec_open, vec_open), calc_theta(vec_open, vec_closed), 
                label="Raised ref.", marker="X", alpha=1, edgecolors="#404040", 
                s=550, lw=3, color="#EAAFCC")
        ax.scatter(calc_theta(vec_open, vec_closed), 0, label="Lowered ref.", 
...
def plot_1dfes(bs_path, rxn_coord, fig_path):
    ...
    labels = {"apo" : "native,\napo state", "holo" : "native,\nholo state",
        "apo_K57G" : "K600G,\napo state", "holo_K57G" : "K600G,\nholo state"}
    ...
    if rxn_coord == "open":
        ax.plot(fes["odot"], fes["ffr1d_open"], label=labels[state], lw=2)
        ax.set_xlabel(f"{rxn_coord_labs[rxn_coord]} (nm$^2$)", labelpad=3,
            fontsize=8)
    elif rxn_coord == "closed":
        ax.plot(fes["cdot"], fes["ffr1d_closed"], label=labels[state], lw=2)
        ax.set_xlabel(f"{rxn_coord_labs[rxn_coord]} (nm$^2$)", labelpad=3,
            fontsize=8)
    elif rxn_coord == "sb":
        ax.plot(fes["sb"], fes["ffr1d_sb"], label=labels[state], lw=2,
            fontsize=8)
        ax.set_xlabel(f"{rxn_coord_labs[rxn_coord]} (nm)", labelpad=3)

    ax.set_ylabel(f"F({rxn_coord_labs[rxn_coord]}) (kJ / mol)", labelpad=3,
        fontsize=8)
