...
def main(argv):
    ...
    try:
        ...
        parser.add_argument("-e", "--extra",
                            ...
                            dest = "extra_contacts",
                            default = False,
                            help = ("Additional contacts to dataframe?"))
    ...
    if recalc or not os.path.exists(df_path) or extra_contacts: 
        ...
        for window in np.arange(1, nw[state] + 1):
            ...
            for run in [1,2,3,4]:
                ...
                if extra_contacts:
                    ...
                    # Add columns for the desired additional contacts
        ...
        if extra_contacts:          
            df.to_csv(f"{ home }/windows_averages_sasa.csv")
        else:
            df.to_csv(df_path)
...
def get_contact_dists(u):
    ...
    from config.settings import selections_k232

    averages = {}

    for key, select in selections_k232.items():

        if ("IP6" in key) & ("holo" not in state):
            continue
        elif ("K57" in key or "K600" in key) & ("K57G" in state):
            continue
...
def get_sasas(u, path, sim_name, holo=False):
    ...
    resids_water = {"W761 SASA" : 218, "E743 SASA" : 200, "H757 SASA" : 214, 
        "E749 SASA" : 206, "D771 SASA" : 228, "E765 SASA" : 222, 
        "D756 SASA" : 213, "R751 SASA" : 208}
    ...
    for key, resid in resids_water.items():
        ...
        if not os.path.exists(sasa_path) or recalc:
...
def plot_averages(df, col, path):
    ...
    if "SASA" in col:
    ...
    elif "K775--IP6" in col:
        print(col)
        d = ax.tricontourf(df[df["Run"] == 1]["OpenPoints"], 
            df[df["Run"] == 1]["ClosedPoints"], ave_val, 10, cmap="coolwarm", 
        ...
        d = ax.tricontourf(df[df["Run"] == 1]["OpenPoints"], 
            df[df["Run"] == 1]["ClosedPoints"], ave_val, 10, cmap="coolwarm", 
            norm=mcolors.Normalize(vmin=2, vmax=12), extend="max")
        # Colormap settings
        cbar = plt.colorbar(d)
        cbar.set_label(f"{ col } " + r"(nm$^2$)", labelpad=3, fontsize=8)
    
    # elif "K775--E796" in col:
    #     print(col)
    #     d = ax.tricontourf(df[df["Run"] == 1]["OpenPoints"], 
    #         df[df["Run"] == 1]["ClosedPoints"], ave_val, 10, cmap="coolwarm", 
    #         norm=mcolors.Normalize(vmin=0, vmax=15))
    ...
    #     cbar.set_label(f"{ col } " + r"(nm$^2$)", labelpad=3, fontsize=8)
        ...
        cbar.set_label(f"{ col } " + r"($\AA$)", labelpad=3,
            fontsize=8)

    # Add axes labels, tick positions 
    plt.xticks([0, 2, 4, 6], fontsize=6)
    plt.yticks([0, 2, 4, 6], fontsize=6)
    ax.set_xlabel(r"$\xi_1$ (nm$^2$)", labelpad=3, fontsize=8)
    ax.set_ylabel(r"$\xi_2$ (nm$^2$)", labelpad=3, fontsize=8)
    ax.set_xlim(0,6)
    ax.set_ylim(0,6)
    ax.set_aspect('equal', adjustable='box')
    cbar.ax.tick_params(labelsize=6, color="#B0B0B0")
    cbar.outline.set_linewidth(1)

    utils.save_figure(fig, f"{ path }/window_ave_{ col }.png")
    plt.close()
...
def plot_windows(df, path):
    ...
    utils.save_figure(fig, f"{ path }/window_labels.png")
    plt.close()
