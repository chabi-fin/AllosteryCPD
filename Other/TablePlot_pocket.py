...
gs = gridspec.GridSpec(n_rows, n_cols, width_ratios=[5,10,5,1,10], 
...
for i, col in enumerate(selections.keys()):
    ...
    if "R752" in col:
        norm = Normalize(vmin=np.min(ave_val_holo), vmax=14)
    else:
        norm = Normalize(vmin=np.min(ave_val_holo), vmax=np.max(ave_val_holo))

    # Third colomn for holo window aves
    holo_ave = plt.subplot(gs[i+1,2])
    ...
    # Spacer column
    space_ax = plt.subplot(gs[i+1,3])
    space_ax.axis('off')

    # Fifth column is for the TcdA histograms
    tcda_ax = plt.subplot(gs[i+1,4])
...
ax2.axis('off')
ax3 = plt.subplot(gs[n_rows - 1,4])
ax3.text(0.5, 0.1, "Distance ($\AA$)", ha='center', va="center", 
    fontsize=10)
ax3.axis('off')

from matplotlib.lines import Line2D
line = Line2D([0.67, 0.67], [0, 1], color='k', linestyle='--', 
    transform=fig.transFigure, figure=fig)
fig.add_artist(line)

#plt.tight_layout()
utils.save_figure(fig, f"{ cf.figure_head }/contacts_table_plot_pocket.png")
plt.show()
