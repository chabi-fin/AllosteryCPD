...
selections = {"E592--W761" : ("resid 218 and name NE1", "resid 49 and name OE*"),
...
tcda_analogues = {"R751--N747" : "R752--N748", "N747--E753" : "N748--E754", 
                ...
                "E592--W761" : "E593--W762", "N596--E592" : "N597--E593",
                "K775--IP6" : "K776--IP6", "K764--IP6" : "K765--IP6",
                "R751--IP6" : "R752--IP6", "R752--IP6" : "K753--IP6",
                "D771--K775" : "D772--K776", "K764--D756" : "K765--A757", 
                "R752--E765" : "K753--E766", "H757 SASA" : "H758 SASA",
                "W761 SASA" : "W762 SASA", "E749 SASA" : "E750 SASA", 
                "E743 SASA" : "E744 SASA"}
...
gs = gridspec.GridSpec(n_rows, n_cols, width_ratios=[6,8,4,5,1,8], 
    height_ratios=([1,6,6,1,6,6,6,6,1]))
...
for i, col in enumerate(selections.keys()):
    ...
    if i < 2:
        ...
        hist_ax.set_xticks([0, 1, 2, 3])
    if col == "N596--E592":
        hist_ax.set_xticklabels([2,5,25], fontsize=8)
    elif col == "E743 SASA":
        hist_ax.set_xticklabels([0,1,2,3], fontsize=8)
    else:
    ...
    if col == "W761 SASA":
        norm = Normalize(vmin=np.min(combo), vmax=1.3)
    elif col == "E743 SASA":
        norm = Normalize(vmin=np.min(combo), vmax=.8)
    elif col == "E592--W761":
        norm = Normalize(vmin=np.min(combo), vmax=21)
    else:
    ...
    if i < 2:
        ...
        tcda_ax.set_xlim(0,3)
        tcda_ax.set_xticks([0, 1, 2, 3])
    if col == "N596--E592":
        tcda_ax.set_xticklabels([2,5,25], fontsize=8)
    elif col == "E743 SASA":
        tcda_ax.set_xticklabels([0,1,2,3], fontsize=8)
    else:
...
# Column xaxis labels
dist_label = plt.subplot(gs[3,1])
dist_label.text(0.5, .1, "Distance ($\AA$)", ha='center', va='center', 
    fontsize=10)
# apo_coord = plt.subplot(gs[3,2])
# apo_coord.text(0.5, .1, "Distance ($\AA$)", 
#     ha='center', va='center', fontsize=10)
# holo_coord = plt.subplot(gs[3,3])
# holo_coord.text(0.5, .1, "Distance ($\AA$)", 
#     ha='center', va='center', fontsize=10) 
dist_label.axis('off')
...
ax3.text(0.5, 0.1, r"($\xi_1 \times \xi_2$)", 
    ha='center', va='top', fontsize=10)
ax3.axis('off')
dist_label_tcda = plt.subplot(gs[3,5])
dist_label_tcda.text(0.5, 0.1, "Distance ($\AA$)", ha='center', va="center", 
    fontsize=10)
dist_label_tcda.axis("off")
sasa_label = plt.subplot(gs[n_rows - 1,1])
sasa_label.text(0.5, 0.1, "SASA (nm²)", ha='center', va='top', 
    fontsize=10)
sasa_label.axis('off')
sasa_label_tcda = plt.subplot(gs[n_rows - 1,5])
sasa_label_tcda.text(0.5, 0.1, "SASA (nm²)", ha='center', va='top', 
    fontsize=10)
...
#plt.tight_layout()
utils.save_figure(fig, f"{ cf.figure_head }/contacts_table_network_suppl.png")
plt.show()
